...
export async function POST(request: Request) {
  try {
    ...
    const { data: txn, error: txnError } = await supabase
      ...
      .select('id, invoice_id, expense_entry_id, confirms_asaas_transaction_id')
    ...
    // 3b. Recalcula a fatura (invoice) — só se não era uma confirmação bancária
    if (invoice_id && !confirms_asaas_transaction_id) {
      ...
      if (inv) {
        ...
        // Exclude bank-statement confirmations from the sum to avoid double-counting
        const sum = (remaining ?? [])
          .filter((r: { confirms_asaas_transaction_id: string | null }) => !r.confirms_asaas_transaction_id)
          .reduce((s, r: { value: number }) => s + Number(r.value), 0);
